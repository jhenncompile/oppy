...
function Chip({ activo, children, ...props }) {
  return (
    <button
      ...
      aria-pressed={activo}
      className={[
        'min-h-[44px] shrink-0 rounded-full border px-4 text-sm transition',
        activo
    ...
    <Panel>
      ...
      {matches.length > 0 && (
        <div className="mb-8 flex flex-col gap-3">
          ...
          <div>
            <Chip activo={soloUrgentes} onClick={() => setSoloUrgentes((s) => !s)}>
      ...
      {cargando ? (
        ...
        <>
          {!mostrarTodas && visibles.length > TOP && (
            <p className="mb-4 text-sm font-medium text-ink">Lo mejor para vos</p>
          )}
